...
app.post('/createReview/:restID', (req, res) => {
  ...
  newReviews.save()
    .then(savedReviews => {
      res.status(201).json(savedReviews); // Respond with the saved review
    })
    .catch(err => {
      res.status(500).json({ error: 'Failed to add review'}); // Handle errors
    });
});
